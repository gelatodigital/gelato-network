...
export default task(
  ...
  .setAction(async ({ fee }) => {
    try {
      ...
      const potentialFeeHandlerAddress = await feeHandlerFactory.feeHandlerByProviderAndNum(
        providerAddress,
        feeNumerator
      );

      if (potentialFeeHandlerAddress !== constants.AddressZero)
        throw Error(
          `FeeHandler is already deployed. Its address is: ${potentialFeeHandlerAddress}`
        );
      ...
      const feeNumeratorAddress = await feeHandlerFactory.feeHandlerByProviderAndNum(
        providerAddress,
        feeNumerator
      );

      console.log(`
      \n FeeHandler Action Address: ${feeNumeratorAddress}
      --------------------------
      \n Fee: ${fee}
