...
describe("Test possible intregration test architecture", () => {
  ...
  it("test10 - Withdraw 2nd Execution Claim - execManualWithdraw", async function() {
    this.timeout(50000);
    await commandLine.execShellCommandLog(
      `truffle test test/manualWithdrawTest.js`
    );
    console.log("#########################################");
  });
  it("test11 - Cancel 3rd Execution Claim - cancelOrder", async function() {
    this.timeout(50000);
    await commandLine.execShellCommandLog(`truffle test test/cancelTest.js`);
    console.log("#########################################");
  });
