...
describe("Cancel outstanding execution claims", () => {
  ...
  it("Cancel Action from someone other than the seller should revert", async () => {
    ...
    txReceipt = await truffleAssert.reverts(
      gelatoDutchExchange.contract.methods
        .cancelOrder(nextExecutionClaim)
  ...
  it("Cancel Claim pair", async () => {
    ...
    // Cancel executionClaim
    txReceipt = await gelatoDutchExchange.contract.methods
      .cancelOrder(nextExecutionClaim)
    ...
    let userSellTokenBalanceAfterBN = new BN(
      userSellTokenBalanceAfter.toString()
    );
