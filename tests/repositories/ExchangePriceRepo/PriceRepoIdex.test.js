const PriceRepoIdex = require('../../../src/repositories/PriceRepo/feeds/PriceRepoIdex')
const priceRepo = new PriceRepoIdex({})
...
test('It should return a price for known Crypto markets', async () => {
  ...
  // It's important because in Idex token order matters
...
test('It should throw an error for unknown Crypto markets', async () => {
  ...
  try {
    ...
    expect(e).toEqual(
      new Error(
        'No matching markets in Idex: XBT-OMG'
      )
    )
  }
})
