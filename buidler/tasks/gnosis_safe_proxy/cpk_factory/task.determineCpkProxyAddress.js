...
export default task(
  ...
  .setAction(async ({ useraddress, saltnonce, log })=> {
    try {
      ...
      const gnosisSafeAddress = ethers.utils.getAddress(
        ethers.utils.solidityKeccak256(['bytes', 'address', 'bytes32', 'bytes32'], [
          '0xff',
          proxyFactory,
          create2Salt,
          ethers.utils.solidityKeccak256(['bytes', 'bytes'], [
            proxyFactoryCreationCode,
            ethers.utils.defaultAbiCoder.encode(['address'], [mastercopy]),
          ]),
        ]).slice(-40),
      );
